public class BlackMoveLogic {
    private Board board;
    private Game game;

    public BlackMoveLogic(Game game, Board board) { 
        this.game = game;
        this.board = board;
    }
    ...
    public boolean isKingCheckedChecker(){
        ...
        if (gamerules.isBlackUnderCheck()) {
            legalmove = false;
            return legalmove;
        }
        else{
    ...
    public boolean Pawn(int startX, int startY, int endX, int endY){
        ...
        if (Character.isLowerCase(boardArray[endY - 1][endX - 1])) {
            System.out.println("Cannot move to a square occupied by your own piece.");
            return false;
            
        }else if ((startY == 7 && endY == startY + 2 && endX == startX) && boardArray[endY - 1][endX - 1] == ' ' || boardArray[endY - 1][endX - 1] == '#' || !(Character.isUpperCase(boardArray[endY - 1][endX - 1]))  ) {
        ...
        }  else if ((endY == startY + 1 && endX == startX) && boardArray[endY - 1][endX - 1] == ' ' || boardArray[endY - 1][endX - 1] == '#' || !(Character.isUpperCase(boardArray[endY - 1][endX - 1]))  ) {
            boardArray[endY - 1][endX - 1] = 'p';
    ...
    boolean isKnightMove = ((Math.abs(startY - endY) == 2 && Math.abs(startX - endX) == 1) || (Math.abs(startY - endY) == 1 && Math.abs(startX - endX) == 2));
        if (isKnightMove) {
    ...
    public boolean Bishop(int startX, int startY, int endX, int endY) {
        ...
        if (isBishopMove) {
            ...
            boardArray[endY - 1][endX - 1] = boardArray[startY - 1][startX - 1];
            clearPrevPos(startX, startY);
            game.incrementMoveCounter();
            return true;
        } else {
